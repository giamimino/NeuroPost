...
export async function POST(req: Request) {
  try {
    ...
    const refreshToken = cookieStore.get(process.env.REFRESH_COOKIE_NAME!)?.value;
    ...
    const payload = jwt.verify(rows[0].token, process.env.REFRESH_SECRET!) as any;

    const newAccessToken = createAccessToken(payload.userId);

    const res = NextResponse.json({ ok: true, rows }, { status: 200 });
    res.cookies.set(process.env.ACCESS_COOKIE_NAME!, newAccessToken, { httpOnly: true });
    return res;
  } catch (err) {
    console.log(err);
