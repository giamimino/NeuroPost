...
const RegisterPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await apiFetch(url, {...ApiConfig.post, body: JSON.stringify({ username, email, password })})
    ...
    <div>
      <div className="w-full h-screen flex justify-center items-center">
        <BlurWrapper>
          ...
          <button type="submit" className="rounded-md bg-foreground text-secondary-bg font-inter py-3 cursor-pointer font-medium text-sm hover:bg-foreground/60">
            Register
          </button>
          </form>
